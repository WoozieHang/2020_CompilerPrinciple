...
int currentDepth=0;
int SemanticErrorNum=0;
struct HashListHead* listHead=NULL;
int finishSemantic=0;
/*
...
void HashInit(){
	...
	t->kind=BASIC;
	t->u.basic=0;
	f->name=(char*)malloc(sizeof(char)*5);
	strcpy(f->name,"read");
	f->rtn_type=t;
	f->v_num=0;
	f->attrList=NULL;
	...
	f->rtn_type=t;
	f->v_num=1;
	f->attrList=(struct FieldList*)malloc(sizeof(struct FieldList));
	f->attrList->line=0;
	f->attrList->tail=NULL;
	f->attrList->v=(struct Variable*)malloc(sizeof(struct Variable));
	f->attrList->v->name=(char*)malloc(sizeof(char)*2);
	strcpy(f->attrList->v->name,"x");
	f->attrList->v->type=(struct Type*)malloc(sizeof(struct Type*));
	f->attrList->v->type->kind=BASIC;
	f->attrList->v->type->u.basic=0;
	HashAdd(FUNCTION,f,0);
		...
		if(HashTable[index]==NULL){
			...
			do{
				if(x->depth==currentDepth&&x->type==VARIABLE&&!strcmp(x->u.v->name,tmp->u.v->name)){
					...
					SemanticErrorNum=SemanticErrorNum+1;
					return false;	
				}
...
int getSize(struct Type* tp){
	if(tp->kind==BASIC)
		return 4;
	else if(tp->kind==ARRAY){
		return getSize(tp->u.array.elem)*(tp->u.array.size);
	}
	else{
		struct FieldList* fl=tp->u.strt->structure;
		int count=0;
		while(fl!=NULL){
			count=count+getSize(fl->v->type);
			fl=fl->tail;
		}
		return count;		
	}
...
int getOffset(char* member,struct Structure* strt){
	struct FieldList* fl=strt->structure;
	int Offset=0;
	while(fl!=NULL){
		if(!strcmp(member,fl->v->name))
			return Offset;
		else
			Offset=Offset+getSize(fl->v->type);
		fl=fl->tail;
	}
	return 0;
}

struct Type* getType(char* member,struct Structure* strt){
	struct FieldList* fl=strt->structure;
	while(fl!=NULL){
		if(!strcmp(member,fl->v->name))
			return fl->v->type;
